...
public:
  FakeUdpPeer() {
    ...
    static_cast<void>(netft::test::set_socket_receive_timeout(socket_, std::chrono::seconds{1}));
  ...
  std::array<std::uint8_t, 8> reply_with(const std::array<std::uint8_t, 36> &record) {
    sockaddr_in client{};
    netft::test::SocketLength client_size = sizeof(client);
    std::array<std::uint8_t, 8> request{};
    const auto received =
        netft::test::receive_from_socket(socket_, request.data(), request.size(), 0,
    ...
    const auto sent =
        netft::test::send_to_socket(socket_, record.data(), record.size(), 0,
                                    reinterpret_cast<sockaddr *>(&client), client_size);
...
TEST(UdpTransportTest, SendsARequestAndReceivesARecord) {
  ...
  ASSERT_EQ(transport.receive(bytes.data(), bytes.size(), 100ms), record.size());
  EXPECT_TRUE(std::equal(record.begin(), record.end(), bytes.begin()));
  EXPECT_EQ(request, (std::array<std::uint8_t, 8>{0x12, 0x34, 0x00, 0x02, 0, 0, 0, 0}));
}
...
TEST(UdpTransportTest, ShutdownInterruptsAWaitAsANonRecordResult) {
  ...
  auto pending_receive = std::async(std::launch::async, [&] {
    return transport.receive(bytes.data(), bytes.size(), std::chrono::seconds{5});
  });
