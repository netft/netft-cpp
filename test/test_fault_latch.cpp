...
TEST(FaultLatch, LaterPublicationCannotReplaceFirstCodeOrMessage) {
  ...
  EXPECT_TRUE(latch.publish(netft::FaultCode::Timeout, "first fault", data_mutex, health));
  EXPECT_FALSE(latch.publish(netft::FaultCode::Socket, "later fault", data_mutex, health));
  ...
  EXPECT_EQ(health.last_error, "first fault");
}
